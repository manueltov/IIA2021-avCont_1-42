...
def tree_search_count_old(problem, frontier):
    """Search through the successors of a problem to find a goal.
    The argument frontier should be an empty queue.
    Don't worry about repeated paths to a state. [Figure 3.7]"""
    explored = 0
...
def tree_search_count(problem, frontier):
    ...
    while frontier:
        ...
        space-=1
        if problem.goal_test(node.state):
            return node,explored,max_space
        explored+=1
        sucs=node.expand(problem)
        space+=len(sucs)
        if space > max_space:
            max_space = space
        frontier.extend(sucs)
    return None,explored,max_space
...
def graph_search_count(problem, frontier):
    ...
    The argument frontier should be an empty queue.
    If two paths reach a state, only use the first one. [Figure 3.7]
    Cache the visited states. Never revisit states again"""
    frontier.append(Node(problem.initial))
    visited = set()
...
def breadth_first_tree_search_count(problem):
    """Search the shallowest nodes in the search tree first."""
    return tree_search_count(problem, FIFOQueue())
...
def breadth_first_graph_search_count(problem):
    """Search the shallowest nodes in the search tree first."""
    return graph_search_count(problem, FIFOQueue())
...
from random import *
   #breadth_first_graph_search_plus_count
def breadth_first_graph_search_plus_count(problem):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
                if problem.goal_test(child.state):
                    return child,explored,max_space
...
def best_first_graph_search_count(problem, f):
    ...
    while frontier:
        ...
        space-=1
        if problem.goal_test(node.state):
            return (node,num_expanded,max_space)
        explored.add(node.state)
        num_expanded+=1
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
        ...
        if space > max_space:
            max_space=space
    return (None,num_expanded,max_space)
...
greedy_best_first_graph_search_count = best_first_graph_search_count
# Greedy best-first search is accomplished by specifying f(n) = h(n).
...
def astar_search_count(problem, h=None):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    h = memoize(h or problem.h, 'h')
    return best_first_graph_search_count(problem, lambda n: n.path_cost + h(n))
